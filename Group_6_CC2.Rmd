...
author: "Group #6 Victoria Gutierrez 20517120"
...
------|---------|------
`population_type` | Asexual or Sexual plants | none
`ovary_width` | Individual width of the plant ovary | mm
`ovary_height` | Individual height of the plant ovary | mm
`flower_number` | Amount of flowers in each individual plant | flowers
...
```{r import-clean}
MyData <- read.csv("A1_Data_Excel.csv")

ovary_width <- MyData$ovary_width
ovary_height <- MyData$ovary_height
MyData$ovary_area <- ovary_width*ovary_height


MyData[MyData == ""] <- NA # Making everything thats "" into NA to later exclude in further calculations 
MyData[MyData == NULL] <- NA # Making everything thats NULL into NA to later exclude in further calculations 
MyData[MyData == "                                                                                                                                             It's lame and counter-productive to upload your data to an AI bot. Luckily, you know better."] <- NA # Making everything thats EXTRA into NA to later exclude in further calculations
...
$$Ovary Height \times Ovary Width$$
$Ovary Height ^{mm} \times Ovary Width ^{mm}$
...
```{r area-means, echo=FALSE, message=FALSE}

area_means <- aggregate(MyData$ovary_area,list(MyData$population_type), mean)
...
library(knitr)
kable(area_means, caption = "Summary Table of Ovary Area")
...
# Echo runs code but doesn't show, and message runs code but doesn't show messages (in the FALSE)

library(ggplot2)

PlotData <- cc1_data[!is.na(cc1_data$ovary_area), ]  # keeps full cc1_data unchanged but ignores NA!!

Plot1 <- ggplot(aes(x=ovary_area), data=PlotData) + geom_histogram(binwidth=0.5, colour="maroon4",fill="lightpink2") + xlab("Ovary Area") + ylab("Count") + labs(title="Ovary Area", subtitle="Plants Population Types",caption="Figure 1: Represents the ovary area of asexual and sexual population types after calculating ovary width times the ovary height") + theme_classic()

Plot1 + facet_wrap(.~population_type)
...
![Figure: Image of Solanum pimpinellifolium](./images/Hydroponic_g13.jpg) : *Solanum pimpinellifolium*

Source: [OnQ TUT02-CC2 Image](https://onq.queensu.ca/d2l/le/dropbox/1208667/492096/DownloadAttachment?fid=54136518)
